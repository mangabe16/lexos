...
class TokenMilestones(BaseModel):
    ...
    doc: Doclike = Field(json_schema_extra={"description": "A string or spaCy Doc object."})

    patterns: Optional[Any] = Field(
        default=None, json_schema_extra={"description": "The pattern(s) used to match milestones."}
    )
    case_sensitive: Optional[bool] = Field(
        default=True, json_schema_extra={"description": "Whether to perform case-sensitive searches."}
    )
    character_map: Optional[dict] = Field(
        default=None, json_schema_extra={"description": "A map of characters to token indexes."}
    )
    attr: Optional[str] = Field(
        default="ORTH",
        json_schema_extra={"description": "The spaCy token attribute to search ('ORTH' or 'LOWER')."}
    )
    flags: Optional[Enum] = Field(
        default=case_sensitive_flags, json_schema_extra={"description": "The regex flags to use."}
    )
    mode: Optional[str] = Field(
        default="string", json_schema_extra={"description": "The mode used to match patterns."}
    )
    nlp: Optional[str] = Field(
        default="xx_sent_ud_sm", json_schema_extra={"description": "The language model to use."}
    )
    type: Optional[str] = Field(default=None, json_schema_extra={"description": "The type of milestone."})
    ...
    def _remove_duplicate_spans(self, spans: list[Span]) -> list[Span]:
        ...
        seen = []
        for span in spans:
            if f"{span.start},{span.end}" not in seen:
                result.append(span)
