...
def test_data_loader_records(data_loader_with_data, sample_paths, sample_mime_types, sample_names, sample_texts):
    """Test DataLoader records property."""
    assert data_loader_with_data.records == [
        {"name": sample_names[0], "path": sample_paths[0], "mime_type": sample_mime_types[0], "text": sample_texts[0]},
        {"name": sample_names[1], "path": sample_paths[1], "mime_type": sample_mime_types[1], "text": sample_texts[1]},
        {"name": sample_names[2], "path": sample_paths[2], "mime_type": sample_mime_types[2], "text": sample_texts[2]}
    ]

def test_data_loader_iter(data_loader_with_data, sample_paths, sample_mime_types, sample_names, sample_texts):
    """Test DataLoader records property."""
...
def test_data_loader_df(data_loader_with_data, sample_paths, sample_mime_types, sample_names, sample_texts):
...
def test_data_loader_load_csv_with_tabs(data_loader, sample_dataframe):
    ...
    assert data_loader.paths == ["csv_string", "csv_string", "csv_string"]
    assert data_loader.mime_types == ["text/tab-separated-values", "text/tab-separated-values", "text/tab-separated-values"]
    assert data_loader.names == ["test1", "test2", "test3"]
    assert data_loader.texts == ["test1", "test2", "test3"]
    assert data_loader.errors == []

def test_data_loader_load_csv_with_invalid_headers_error(data_loader, sample_dataframe_invalid_data):
    """Test DataLoader load_csv method with invalid headers."""
    csv = StringIO(sample_dataframe_invalid_data.to_csv(index=False))
    with pytest.raises(LexosException, match="CSV and TSV files must contain headers named `name` and `text`."):
        data_loader.load_csv(csv)

def test_data_loader_load_csv_with_invalid_headers(data_loader, sample_dataframe_invalid_data):
...
def test_data_loader_load_dataset(data_loader_with_data):
    ...
    assert data_loader_with_data.paths == ["test1.txt", "test2.txt", "test3.txt", "test4.txt", "test5.txt", "test6.txt"]
    assert data_loader_with_data.mime_types == ["text/plain", "text/plain", "text/plain", "text/plain", "text/plain", "text/plain"]
    assert data_loader_with_data.names == ["test1", "test2", "test3", "test4", "test5", "test6"]
    assert data_loader_with_data.texts == ["test1", "test2", "test3", "test4", "test5", "test6"]
    assert data_loader_with_data.errors == []
...
def test_data_loader_load_json(data_loader, sample_dataframe):
    ...
    assert data_loader.mime_types == ["application/json", "application/json", "application/json"]
    assert data_loader.names == ["test1", "test2", "test3"]
    assert data_loader.texts == ["test1", "test2", "test3"]
    assert data_loader.errors == []

def test_data_loader_load_json_with_invalid_headers_error(data_loader, sample_dataframe_invalid_data):
...
def test_data_loader_load_json_with_invalid_headers(data_loader, sample_dataframe_invalid_data):
