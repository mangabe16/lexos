...
def test_save_text_custom_names(cutter_save, output_dir):
    ...
    names = ["test1", "test2"]
    cutter_save.save_text(output_dir, names=names)
...
def test_save_text_custom_format(cutter_save, output_dir):
    ...
    cutter_save.save_text(output_dir, delimiter="-", pad=4)
...
def test_save_text_strip_whitespace(cutter_save, output_dir, nlp):
    ...
    cutter_save.save_text(output_dir, strip_chunks=True)
...
def test_save_text_mismatched_names(cutter_save, nlp, output_dir):
    ...
    with pytest.raises(LexosException, match="must equal"):
        cutter_save.save_text(output_dir, names=["single_name"])
...
def test_split_by_newline(cutter, nlp):
    """Test splitting by newlines."""
    doc = nlp("Line one\nLine two\nLine three")
    cutter.split(doc, n=2, newline=True)
...
def test_split_on_linebreaks_basic(cutter, doc_with_lines):
    """Test basic line splitting with n=2."""
    cutter.split_on_linebreaks(doc_with_lines, n=2)
...
def test_split_on_linebreaks_multiple_docs(cutter, nlp):
    """Test splitting multiple docs."""
    docs = [nlp("Line1\nLine2\n"), nlp("Line3\nLine4\n")]
    cutter.split_on_linebreaks(docs, n=1)
    assert len(cutter.chunks) == 2
    assert len(cutter.chunks[0]) == 2


def test_split_on_linebreaks_merge_threshold(cutter, doc_with_lines):
    """Test merge threshold functionality."""
    print(doc_with_lines)
    cutter.split_on_linebreaks(doc_with_lines, n=2, merge_threshold=0.75)
...
def test_split_on_linebreaks_overlap(cutter, doc_with_lines):
    """Test overlap functionality."""
    cutter.split_on_linebreaks(doc_with_lines, n=2, overlap=1)
    assert (
        cutter.chunks[0][0][-1].text.strip()
        == cutter.chunks[0][1][0].text.strip()
    )


def test_split_on_linebreaks_strip(cutter, nlp):
    """Test stripping whitespace."""
    doc = nlp("  Line1  \n  Line2  \n")
    cutter.split_on_linebreaks(doc, n=1, strip_chunks=True)
    assert cutter.chunks[0][0].text.strip() == "Line1"


def test_split_on_linebreaks_custom_names(cutter, doc_with_lines):
...
def test_split_on_linebreaks_no_newlines(cutter, nlp):
    """Test document with no line breaks."""
    doc = nlp("No line breaks here")
    cutter.split_on_linebreaks(doc, n=1)
...
def test_split_on_milestones_basic(cutter, doc):
    ...
    milestones = [doc[1:2], doc[4:5]]  # "quick", "jumps"
    cutter.split_on_milestones(doc, milestones)
...
def test_split_on_milestones_multiple_docs(cutter, nlp):
    ...
    cutter.split_on_milestones(docs, milestone)
    assert len(cutter.chunks) == 2
...
def test_split_on_milestones_keep_spans(cutter, doc, milestone_doc):
    ...
    cutter.split_on_milestones(doc, milestones, keep_spans=False)
    assert cutter.chunks[0][1].text.strip() == "brown fox"
    assert cutter.chunks[0][2].text.strip() == "over the lazy dog."
    cutter.chunks = []  # Reset chunks
    cutter.split_on_milestones(doc, milestones, keep_spans="following")
    ...
    cutter.chunks = []  # Reset chunks
    cutter.split_on_milestones(doc, milestones, keep_spans="preceding")
    # The quick, brown fox jumps, over the lazy dog.
...
def test_split_on_milestones_strip(cutter, nlp):
    ...
    cutter.split_on_milestones(doc, milestone, strip_chunks=True)
...
def test_split_on_milestones_custom_names(cutter, doc, nlp):
    ...
    milestone = nlp("with")[0:1]
    names = ["test1"]
    cutter.split_on_milestones(doc, milestone, names=names)
    assert cutter.names == names
...
def test_split_on_sentences_overlap(cutter, doc_with_sentences2):
    ...
    assert (
        cutter.chunks[0][0].text.strip() == "First sentence. Second sentence. Third"
    )
    assert (
        cutter.chunks[0][1].text.strip() == "Third sentence. Fourth sentence. Fifth"
    )
