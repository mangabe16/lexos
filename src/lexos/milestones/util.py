...
def filter_doc(doc: Doc, spans: list[Span]) -> Doc:
    ...
    if not Token.has_extension("milestone_iob") or not Token.has_extension("milestone_label"):
    ...
    new_user_data = {
        key: value for key, value in doc.user_data.items()
        if not isinstance(key, tuple) and key[1] not in ["milestone_iob", "milestone_label"]
    }
