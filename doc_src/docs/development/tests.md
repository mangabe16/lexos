...
Lexos uses the [`pytest`](http://doc.pytest.org/) framework for testing. Tests for Lexos modules and classes live in their own directories of the same name and all test files should be prefixed with `test_`.
