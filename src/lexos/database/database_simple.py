...
class CorpusDatabase:
    ...
    def _db_record_to_record(
        ...
        """Convert a DatabaseRecord back to a Record object."""
    ...
    def _record_to_db_record(self, record: Record) -> DatabaseRecord:
        """Convert a Record object to DatabaseRecord for database storage."""
    ...
    def add_record(self, record: Record) -> None:
        ...
        with self.SessionLocal() as session:
            ...
            existing = (
                session.query(DatabaseRecord)
                .filter(DatabaseRecord.id == str(record.id))
                .first()
            )
            if existing:
            ...
            # Convert Record to DatabaseRecord
    ...
    def delete_record(self, record_id: str) -> bool:
        ...
        with self.SessionLocal() as session:
            record = (
                session.query(DatabaseRecord)
                .filter(DatabaseRecord.id == record_id)
                .first()
            )
            if record:
    ...
    def filter_records(
        ...
        with self.SessionLocal() as session:
            query = session.query(DatabaseRecord)

            if is_active is not None:
                query = query.filter(DatabaseRecord.is_active == is_active)
            if is_parsed is not None:
                query = query.filter(DatabaseRecord.is_parsed == is_parsed)
            if model is not None:
                query = query.filter(DatabaseRecord.model == model)
            if min_tokens is not None:
                query = query.filter(DatabaseRecord.num_tokens >= min_tokens)
            if max_tokens is not None:
                query = query.filter(DatabaseRecord.num_tokens <= max_tokens)
    ...
    def filter_records(
        ...
        with self.SessionLocal() as session:
            ...
            records = []
            for row in result:
                # Convert row to DatabaseRecord manually
                db_record = DatabaseRecord()
                for i, col in enumerate(DatabaseRecord.__table__.columns):
