...
def test_mann_whitney_initialization_valid_data(sample_dataframes):
    ...
    mw = MannWhitney(x=x_df, y=y_df)
    
    assert isinstance(mw.x, pd.DataFrame)
    assert isinstance(mw.y, pd.DataFrame)
...
def test_mann_whitney_initialization_add_freq_false(sample_dataframes):
    ...
    mw = MannWhitney(x=x_df, y=y_df, add_freq=False)
...
def test_mann_whitney_with_frequency_stats(sample_dataframes):
    ...
    mw = MannWhitney(x=x_df, y=y_df, add_freq=True)
    result_df = mw.to_df()
    ...
    assert all(isinstance(val, str) and val.endswith('%') for val in result_df['difference'])
...
def test_mann_whitney_invalid_input_types():
    ...
    with pytest.raises(ValidationError):
        MannWhitney(x="not_a_dataframe", y=pd.DataFrame())
    
    with pytest.raises(ValidationError):
        MannWhitney(x=pd.DataFrame(), y="not_a_dataframe")
...
@patch('wasabi.msg.warn')
def test_mann_whitney_missing_columns_warning(mock_warn, dataframes_with_missing_columns):
...
def test_get_freq_stats(sample_dataframes):
    ...
    assert all(isinstance(val, str) and val.endswith('%') for val in freq_stats['difference'])
...
def test_mann_whitney_end_to_end_workflow(sample_dataframes):
    ...
    assert all(col in result_df.columns for col in ['term', 'statistic', 'p_value', 'ave_freq', 'difference'])
...
def test_mann_whitney_no_matching_columns():
    ...
    with patch('wasabi.msg.warn') as mock_warn:
        ...
        mock_warn.assert_any_call("Warning: No suitable columns found to perform the Mann-Whitney U test.")
