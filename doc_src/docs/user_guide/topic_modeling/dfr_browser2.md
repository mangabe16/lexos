...
Calling the `serve()` method will start a localhost serve and open the browser in your system's default web browser.

!!! Note
    DFR Browser 2 must be served from a server; otherwise it will not have full functionality. If you have another server running on the default port of 8000, you can set DFR Browser to run on a different port number with the `port` keyword. This may be important if you are creating your `Browser` instance in a Jupyter notebook that is running on port 8000.
...
The `data_path` is the path to your original training data. It should be a tab-separated file with 2 columns per line (ID, content) or 3 columns per line (ID, label, content) — lines are validated during initialization and copied to `data/docs.txt`.

Because DFR Browser 2 is a separate package, Lexos may not have the latest distribution. If this is the case, you can download the latest version from the [DFR Browser 2 repository](https://github.com/scottkleinman/dfr-browser2) and set the `template_path` to this version.
...
Every instance of DFR Browser 2 has a `config.json` file containing the browser configuration. You can pass configuration values as a dictionary to this file using the `Browser` class with the `config` parameter. For discussion of the configuration options, see the [DFR Browser 2 repository](https://github.com/scottkleinman/dfr-browser2).

As mentioned earlier, you can also set the port on which the browser is served with the `port` parameter.

### Typical usage flow

- Construct the `Browser` object — initialization checks required files, copies the template, copies files into `data/`, validates TSVs, and writes `config.json`.
- Start serving using `b.serve()` — this will run a background HTTP server and open the browser (or print instructions if web browser cannot be opened).
...
- `metadata.csv`
- `topic-keys.txt`
- `doc-topic.txt` (or `doc-topics.txt`)  — synonyms for the documents-to-topic mapping
- `topic-state.gz` (or `state.gz`)  — supports `state.gz` as an alternate name
- `topic_coords.csv` (or `topic-coords.csv`)  — coordinates for topic display
