...
def large_text():
    ...
    base_text = "This is sentence number {}. It contains various words and punctuation! "
    return "".join(base_text.format(i) for i in range(100))  # Reduced for faster testing
...
class TestMissingModelDuringDeserialization:
    ...
    def test_no_model_specified_error(self, nlp, simple_text):
        ...
        with pytest.raises(LexosException, match="No model specified"):
            new_record.from_bytes(serialized_bytes)  # No model provided and none in data
...
class TestSerializationComprehensiveValidation:
    ...
    def test_all_serialization_methods_consistency(self, nlp, complex_text):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for i, (bytes_token, disk_token) in enumerate(zip(bytes_record.content, disk_record.content)):
