...
class DfrBrowser(BaseModel):
    ...
    model_dir: str = Field(
        ".", json_schema_extra={"description": "The directory containing the model."}
    )
    model_state_file: str = Field("state.gz", json_schema_extra={"The state file."})
    model_scaled_file: str = Field(
        "topic_scaled.csv", json_schema_extra={"The scaled file."}
    )
    template_dir: str = Field(
        TEMPLATE_DIR, json_schema_extra={"The template directory."}
    )
    browser_dir: str = Field(None, json_schema_extra={"The browser directory."})
    data_dir: str = Field(None, json_schema_extra={"The data directory."})
    num_topics: int = Field(None, json_schema_extra={"The number of topics."})

    def __init__(self) -> None:
        """Initialize DfrBrowser object."""
        super().__init__()
        self.model_state_file = f"{self.model_dir}/{self.model_state_file}"
        self.model_scaled_file = f"{self.model_dir}/{self.model_scaled_file}"
        self.browser_dir = f"{self.model_dir}/dfr_browser"
    ...
    def run(self, port: int = 8080) -> None:
        ...
        NOTE: This might work on the Jupyter port, but it might not.
        """
