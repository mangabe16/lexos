...
- The split methods have a `merge_final` parameter to force the last chunk to be
  ...
  trailing single lines, sentences, etc. I'm not sure if there is a use case for
  implementing an overlap method that is not based on tokens counts.
"""
...
class TokenCutter(BaseModel, validate_assignment=True):
    ...
    names: Optional[list[str]] = Field(
        default=[], json_schema_extra={"description": "A list of names for the source docs."}
    )
    ...
    merge_threshold: Optional[float] = Field(
        default=0.5, ge=0, le=1, json_schema_extra={"description": "The threshold to merge the last segment."}
    )
    overlap: Optional[int] = Field(
        default=None, gt=0, json_schema_extra={"description": "The number of tokens to overlap."}
    )
    output_dir: Optional[Path | str] = Field(
        default=None, json_schema_extra={"description": "The output directory to save the chunks to."}
    )
    delimiter: str = Field(
        default="_", json_schema_extra={"description": "The delimiter to use for the chunk names."}
    )
    pad: int = Field(default=3, gt=0, json_schema_extra={"description": "The padding for the chunk names."})
    strip_chunks: bool = Field(
        default=True,
        json_schema_extra={"description": "Whether to strip leading and trailing whitespace in the chunks."}
    )
    ...
    def split_on_linebreaks(
        ...
        """Split spaCy docs into chunks by a fixed number of lines.
    ...
    def split_on_milestones(
        ...
        Args:
            docs (Doc | list[Doc]): The document to be split.
    ...
    def to_dict(self, names: Optional[list[str]] = None) -> dict[str, list[str]]:
        ...
        Args:
            names (Optional[list[str]]): A list of names for the source Docs.
